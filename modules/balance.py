...
class Balance:
    ...
    def __init__(self, Puppeteer):
        ...
        if not self._config['USE_WEBSOCKET'] and not self._config['USE']['BALANCE']:
            self._logger.warning('balance output is None. because [not websocket] and [not use balance]')
        ...
        # 資産状況通知スレッド
        # -------------------------------------------------------
        self._send_balance_thread = threading.Thread(target=self.__run, args=('check_balance',))
    ...
    def balance(self):
        if self._config['USE_WEBSOCKET']:
            ...
            # websocket 無効
            balance = self._exchange.fetch_balance() if self._config['USE']['BALANCE'] == True else 0
        ...
        while True:
            ...
            try:
                ...
                if self._walletBalance != walletBalance :
                    ...
                    #         time, balance, diff
                    # --------------------------------------------------
                    message = '{}, {}, {}'.format(datetime.now().strftime("%Y-%m-%d %H:%M:%S"), '{:.8f}'.format(self._walletBalance), '{:.8f}'.format(diff))
                    # --------------------------------------------------
                    ...
                    #         time, balance, diff
                    # --------------------------------------------------
                    message = '{}: balane={}, diff={}'.format(datetime.now().strftime("%Y-%m-%d %H:%M:%S"), '{:.8f}'.format(self._walletBalance), '{:.8f}'.format(diff))
                    # --------------------------------------------------
                    self._discord.send(message=message, fileName='logs/' + self._balanceLogName + '.png')
