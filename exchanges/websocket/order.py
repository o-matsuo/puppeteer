...
class Order:
    ...
    __COLUMN_SETS = "orderID TEXT primary key, " "obj TEXT NOT NULL, " "tm TIMESTAMP"
    ...
    def replace(self, data):
        ...
        try:
                ...
                list.append(
                    tuple(
                        [
                            ...
                            json.dumps(
                                row
                            ),  # 一度TEXTに変換して入れる。sqlite3はver3.9以降でJSONデータをカラムに持てるが、AWS Cloud9のsqlite3はver3.7
            ...
            c.executemany(
                "REPLACE INTO " + Order.__TABLE_NAME + " VALUES (?,?,?)", list
            )
        except Exception as e:
            self.logger.error(e)
            self.rollback(c)
    ...
    def delete(self, data):
        ...
        try:
            list = []
            for row in data:
                list.append(tuple([row["orderID"]]))
            #                                                   params: list [(orderID), (...)]
            c.executemany(
                "DELETE FROM " + Order.__TABLE_NAME + " WHERE orderID = ?", list
            )
        except Exception as e:
            self.logger.error(e)
            self.rollback(c)
    ...
    def select(self, orderID):
        ...
        try:
            ...
            list = c.fetchall()
            for row in list:
                data.append(json.loads(row[1]))  # TEXTをJSONに変換
        except Exception as e:
            self.logger.error(e)
            self.rollback(c)
        else:
